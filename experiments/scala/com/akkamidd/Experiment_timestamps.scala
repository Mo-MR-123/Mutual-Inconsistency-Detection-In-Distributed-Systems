...
class Experiment_timestamps extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  "Experiment" must {
    "Detect Inconsistency" in {
          ...
          for (siteIdx <- 1 to numSites) {
            ...
            for (runIdx <- 1 to numRuns) {
              ...
              masterSite.log.info("Experiment Timestamp ended - time: " + estimatedTime.toString)
